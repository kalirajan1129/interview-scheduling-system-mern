...
exports.sendEmails = async (req, res) => {
  try {
    ...
    for (let i = 0; i < interview.candidates.length; i++) {
       ...
       if (candidate.mailSent) continue; // Skip if already sent

       // The frontend URL for the booking page
       ...
       const emailHtml = `
         <div style="font-family: Arial, sans-serif; padding: 20px; background-color: #f4f4f5; max-width: 600px; margin: 0 auto; border-radius: 8px;">
           ...
           <p>Best regards,<br/>InterviewHub Team</p>
         </div>
       `;

       try {
         // Attempt to send email real/mock
         if (process.env.EMAIL_USER && process.env.EMAIL_PASS) {
            ...
            console.log(`[Mock Email] To: ${candidate.email} | Link: ${bookingLink}`);
         }
         
         // Only mark as sent if the above code doesn't throw an error!
